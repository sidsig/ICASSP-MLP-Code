...
train :
  ...
  def train(self, gradient_dataset, cg_dataset, initial_lambda=0.1, mu=0.03, global_backtracking=False, preconditioner=False, 
            max_cg_iterations=250, num_updates=100, validation=None, validation_frequency=1, patience=numpy.inf, output_folder=None):
  ...
  save_progress: string or None
    ...
    self.save_path = os.path.join(self.output_folder,'best_params.pickle')

    if isinstance(self.output_folder, str) and os.path.isfile(self.save_path):
      save = cPickle.load(file(self.save_path))
      ...
      for u in xrange(first_iteration, 1 + num_updates):
        ...
        if validation is not None and u % validation_frequency == 0:
          ...
          if isinstance(validation,bool):
            cost_valid = numpy.mean(costs,axis=0)
            if cost_valid < best[1]:
        ...
        if isinstance(output_folder, str):
          ...
          cPickle.dump(save, file(save_path, 'wb'), cPickle.HIGHEST_PROTOCOL)
